...
## Часть 10. Ограничение частоты

> Дописано вместе с частью 9.

В части 4.3 троттлинг считает неудачи **по имени пользователя**. У него есть слепое пятно, которое
видно сразу, как только его назвать: атакующий, подставляющий каждый раз **новое** имя, оставляет
каждый счётчик на единице. Счётчиков много, ни один не срабатывает.

И второе, более неприятное: регистрацию агентов не считал вообще никто. А токен установки — это
**одно общее значение, которое не истекает**. Единственное, что отделяло его от подбора, — скорость
сети.

### 10.1. Встроенный ограничитель

В ASP.NET Core с 7-й версии есть `AddRateLimiter`, и писать своё не нужно:
...
options.AddPolicy(Enrollment, context => RateLimitPartition.GetFixedWindowLimiter(
    ...
    {
        PermitLimit = enrollmentPerHour,
        Window = TimeSpan.FromHours(1)
    }));
```

**Раздел (partition)** — ключ, по которому считается лимит; здесь адрес отправителя. Политика
вешается на конкретный эндпоинт атрибутом `[EnableRateLimiting(...)]`.

Кроме `FixedWindow` есть скользящее окно, «дырявое ведро» и токен-бакет. Фиксированное окно —
самое грубое из них (на стыке двух окон можно проскочить двойную порцию) и здесь достаточное:
задача не «пропустить ровно N», а «превратить тысячу попыток в секунду в десять в час».

### 10.2. Три решения, каждое спорное

**Код ответа 429, а не 503.** «Слишком много запросов» говорит клиенту «притормози», а
«сервис недоступен» — «переключайся на резерв». Разные смыслы и разное поведение вызывающего.

**Раздел по IP-адресу — заведомо несовершенен.** Все за одним NAT делят корзину, а у атакующего с
ботнетом корзин столько же, сколько адресов. Это не оправдание, а честный размен: ограничение
всё равно превращает неограниченную скорость подбора в ограниченную, а слепое пятно закрывает
троттлинг по имени. **Две несовершенные защиты, закрывающие слабости друг друга, — обычная и
рабочая конструкция.**

**Лимит считает и успешные попытки тоже.** Считать только неудачи означало бы, что подобравший
токен регистрирует машины без ограничений.

### 10.3. Что намеренно не ограничено

Приём метрик (`/api/ingest`). Агент уже держит выданный ему ключ, он ничего не угадывает — и
ограничение здесь только роняло бы данные машин, которые ведут себя правильно.

> **Правило.** Ограничение частоты ставят туда, где принимают **догадку**, а не туда, где
> принимают предъявленный секрет.
...
- Ограничение частоты ставится туда, где принимают догадку, а не предъявленный секрет.
- Две несовершенные защиты, закрывающие слепые пятна друг друга, лучше одной «правильной».
