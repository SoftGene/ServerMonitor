...
> **Как это исправили.** Долгое время `try/catch` в этом обработчике не было: упади JS-вызов
> (например, элемент ещё не отрисован), исключение осталось бы необработанным. Сейчас тело
> обёрнуто в `try/catch` с записью в журнал, а отдельно ловится `JSDisconnectedException` —
> ситуация, когда соединение с браузером уже закрыто и обновлять просто нечего. Разбор — в
