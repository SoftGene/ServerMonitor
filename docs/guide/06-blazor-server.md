...
> **Как это исправили.** Раньше вызовы JS не были обёрнуты в `try/catch`, и в `TopNav` это
> усугублялось тем, что обработчик объявлен как `async void` (глава 01) — исключение оттуда
> никто не поймал бы. Теперь вызов индикатора вынесен в отдельный метод с обработкой
> `JSDisconnectedException`, а сам обработчик события ловит остальные исключения и пишет
...
- ~~**`async void` в `TopNav`** и **JS-вызовы без обработки ошибок**~~ — исправлено, см.
  часть 6.
