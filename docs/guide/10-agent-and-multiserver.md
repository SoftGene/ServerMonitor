...
> **Слабое место.** Буфер живёт **в памяти**. Если агента перезапустить, пока связи нет,
> накопленные замеры пропадут. Настоящее решение — писать их на диск; для домашнего парка
> это пока избыточно, но знать про ограничение нужно.
...
> **Чего здесь не хватает до промышленного решения.** Обычно к выдержке добавляют **дрожание
> (jitter)** — случайную добавку к паузе. Без него все агенты, потерявшие связь одновременно,
> и повторять будут одновременно, синхронным залпом. Для нашего масштаба несущественно, для
> сотни машин — обязательно.
...
**Буфер агента живёт в памяти.** См. раздел 5.2.
