...
**Токен установки бессрочный, ключи не отзываются.** См. раздел 4.2. Подбор токена с тех пор
ограничен по частоте (глава 12, часть 10) — это уменьшает размер проблемы, но не убирает её.
