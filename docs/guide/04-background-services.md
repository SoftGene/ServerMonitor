...
```csharp
_interval = options.Value.CollectInterval;   // из секции "Monitoring" конфигурации
```
...
- ~~**Интервалы зашиты в код.**~~ Исправлено: `TimeSpan.FromSeconds(5)` и `FromSeconds(30)`
  были константами в полях классов, и поменять частоту сбора можно было только
  перекомпиляцией. Теперь оба интервала приходят из секции `Monitoring` конфигурации через
  `IOptions<MonitoringOptions>` — см. главу 08 и [главу 09](09-fixing-the-defects.md).
